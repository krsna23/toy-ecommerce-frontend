...
export class ProductService {
  ...
  getfirstToys() {
      ...
      return this.firstToys;
  }

  getFirstId(id:number){
    return this.firstToys[id -1];
  }
  ...
  private baseUrl = 'http://localhost:3000/toys';

  // Method to get all products from the backend
  getAllProducts(): Observable<Toy[]> {
    return this.http.get<Toy[]>(this.baseUrl);
  }

  // Method to get a product by ID from the backend
  getProductById(id: number): Observable<Toy> {
    const url = `${this.baseUrl}/${id}`;
    return this.http.get<Toy>(url);
  }

  getProductsByUserId(userId: number): Observable<Toy[]> {
